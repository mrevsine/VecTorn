...
public:
   ...
   size_t serialize(std::ostream &out)
   {
      ...
      size_t size = salt_.size();
      out.write((char *)&size, sizeof(size));
      written_bytes += sizeof(size);

      for(size_t i = 0; i < size; ++i)
      {
         out.write((char *)&salt_[i], sizeof(salt_[i]));
         written_bytes += sizeof(salt_[i]);
      }

      size = bit_table_.size();
      out.write((char *)&size, sizeof(size));
      written_bytes += sizeof(size);

      for(size_t i = 0; i < size; ++i)
      {
         out.write((char *)&bit_table_[i], sizeof(bit_table_[i]));
         written_bytes += sizeof(bit_table_[i]);
      }

      out.write((char *)&salt_count_, sizeof(salt_count_));
      written_bytes += sizeof(salt_count_);

      out.write((char *)&table_size_, sizeof(table_size_));
      written_bytes += sizeof(table_size_);

      out.write((char *)&projected_element_count_, sizeof(projected_element_count_));
      written_bytes += sizeof(projected_element_count_);

      out.write((char *)&inserted_element_count_, sizeof(inserted_element_count_));
      written_bytes += sizeof(inserted_element_count_);

      out.write((char *)&random_seed_, sizeof(random_seed_));
      written_bytes += sizeof(random_seed_);

      out.write((char *)&desired_false_positive_probability_, sizeof(desired_false_positive_probability_));
      written_bytes += sizeof(desired_false_positive_probability_);
   ...
   void load(std::istream &in)
   {
      ...
      salt_ = std::vector<bloom_type>(size);
      for(size_t i = 0; i < size; ++i)
      {
         in.read((char *)&salt_[i], sizeof(salt_[i]));
      }

      in.read((char *)&size, sizeof(size));
      bit_table_ = std::vector<unsigned char>(size);
      for(size_t i = 0; i < size; ++i)
      {
         in.read((char *)&bit_table_[i], sizeof(bit_table_[i]));
      }

      in.read((char *)&salt_count_, sizeof(salt_count_));
      in.read((char *)&table_size_, sizeof(table_size_));
      in.read((char *)&projected_element_count_, sizeof(projected_element_count_));
      in.read((char *)&inserted_element_count_, sizeof(inserted_element_count_));
