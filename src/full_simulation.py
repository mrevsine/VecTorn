...
# create random SNP in a genomic sequence
# choose a random locus and a random base to replace it with
def create_random_SNP(seq, seed=1):
  random_locus = random.randrange(len(seq))
  bases = ["A","C","G","T"]
  new_bases = [base for base in bases if base != seq[random_locus]]
  random_base = new_bases[random.randrange(3)]
  return seq[:random_locus] + random_base + seq[random_locus+1:]
